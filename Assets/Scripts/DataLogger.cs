...
public class DataLogger : MonoBehaviour
{
    ...
    [Header("Joystick Input (for movement gate)")]
    public InputActionProperty joystickAction;   // bind to same action as PointerRotation
    ...
    private bool joystickMovedThisTrial = false;
    ...
    private void Start()
    {
        if (trialManager == null)
            Debug.LogError("[DataLogger] TrialManager not assigned in Inspector.");

        if (joystickAction.action != null)
    ...
    private void Update()
    {
        ...
        if (!joystickMovedThisTrial && joystickAction.action != null)
        {
            Vector2 input = joystickAction.action.ReadValue<Vector2>();

            if (Mathf.Abs(input.x) > joystickDeadzone)
            {
                joystickMovedThisTrial = true;
                Debug.Log("[DataLogger] Joystick moved this trial.");
            }
        }
        ...
        {
            if (!joystickMovedThisTrial)
            {
                Debug.Log("[DataLogger] Move joystick before pressing A.");
                TriggerRightHaptics(0.2f, 0.05f);
                return;
            }

            SaveCurrentAngleOnly();
    ...
    private void SetupFile(string id)
    {
        ...
        string header = "Time,TrialNumber,ConditionID,Distance,SphereSide,StartAngle,MeasuredAngle,HMDHeight,SavedAngleTime";
    ...
    public void FinalizeTrialAndWrite()
    {
        ...
        if (practiceMode)
        {
            Debug.Log("[DataLogger] Practice mode: not writing to CSV.");
